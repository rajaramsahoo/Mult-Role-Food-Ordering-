...
const ResturantOwnerSidebar = () => {
  const [open, setOpen] = useState(true);
  const navigate = useNavigate();

  const Menus = [
    { title: "Home", icon: <Home size={20} />, navigate: "/" },
    { title: "Dashboard", icon: <LayoutDashboard size={20} />, navigate: "/restaurant-owner-dashboard" },
    { title: "Users", icon: <Users size={20} />, navigate: "/restaurant-owner/partner", gap: true },
    { title: "Restaurants", icon: <Utensils size={20} />, navigate: "/restaurant-owner/partner-with-us" },
    { title: "Menu", icon: <List size={20} />, navigate: "/restaurant-owner/menu" },
    { title: "Orders", icon: <ShoppingCart size={20} />, navigate: "/restaurant-owner/orders" },
  ...
  return (
    <div className={` ${open ? "w-72" : "w-20"} bg-orange h-screen p-5 pt-8 relative duration-300 flex flex-col justify-between `}>
      ...
      <div className={`flex gap-x-4 items-center ${!open && "hidden"}`}>
        ...
        />
        <h1 className={`text-white text-xl font-medium origin-left duration-200 ${!open && "scale-0"} overflow-hidden`}>
          Resturant Owner Panel
        </h1>
      </div>
      ...
      <div className="border-t border-white pt-4 flex items-center gap-3 mt-auto">
        ...
        <div className={`flex flex-col ${!open && "hidden"} transition-all`}>
          <h4 className="font-bold text-white">Resturant Owner name</h4>
...
export default ResturantOwnerSidebar